...
\begin{frame}[plain]
  ...
  \begin{tikzpicture}[font=\sffamily,line cap=round,line join=round]
    ...
    \node[fill=white,draw=Navy!16,rounded corners=1.5pt,inner xsep=4pt,
          inner ysep=2.4pt,align=center,text=Ink,font=\bfseries\scriptsize,
          anchor=south] at (7.35,0.96) {Diffusion\\samplers};
    \node[fill=white,draw=Navy!16,rounded corners=1.5pt,inner xsep=4pt,
          inner ysep=2.4pt,align=center,text=Ink,font=\bfseries\scriptsize,
...
\begin{frame}{PushT: one initial state, two rotation modes}
  ...
  {\small Same initial state ($192^\circ$), same \textbf{DA:REPPO} policy.\\
   Different diffusion-prior samples $\rightarrow$ two successful strategies.}
\end{frame}
  ...
  \begin{columns}[c,onlytextwidth]
    \column{0.46\textwidth}
      \begin{itemize}
        ...
        \item<1-> \textbf{Given:} cost $C(x)$; lower is better.
        \item<2-> \textbf{Boltzmann target} ($\Tau>0$):
          \[
            p_{\Tau}(x)=\frac{e^{-C(x)/\Tau}}{Z_{\Tau}},
          \]
          \[
            Z_{\Tau}=\int e^{-C(x)/\Tau}\,dx<\infty.
          \]
        \item<3-> \textbf{Approximate} $p_{\Tau}$ with tractable
          $q_\theta$ (e.g., a Gaussian).
        \item<4-> \textbf{Fit} $q_\theta$ by minimizing its KL divergence
          to $p_{\Tau}$.
      \end{itemize}
...
\begin{frame}{Reparameterization trick: a Gaussian example}
  ...
  \begin{itemize}[<+->]
    \item \textbf{Start with a Gaussian:}
      \[
        q_\theta=\Normal(\mu,\sigma^2),
        \qquad \theta=(\mu,\sigma),\quad \sigma>0.
      \]
    \item \textbf{Sample standard noise, then shift and scale:}
      \[
        \varepsilon_i\sim\Normal(0,1),
        \qquad \boxed{X_i=\mu+\sigma\varepsilon_i\sim q_\theta.}
      \]
    \item \textbf{Hold $\varepsilon_i$ fixed during backpropagation:}
      for differentiable $R$,
      \[
        \frac{\partial R(X_i)}{\partial\mu}=R'(X_i),
        \qquad
        \frac{\partial R(X_i)}{\partial\sigma}
        =R'(X_i)\,\varepsilon_i.
      \]
    \item \textbf{Gaussian VI loss} (samples via \texttt{q.rsample()}):
      \[
        \boxed{\widehat L_{\mathrm{path}}
        =-\frac1N\sum_{i=1}^{N}R(\mu+\sigma\varepsilon_i)
         -\Tau\underbrace{\tfrac12\log(2\pi e\sigma^2)}_{
      ...
      \begin{itemize}
        ...
        \item<1-> \textbf{Start with data} $X^0\sim p_0$; choose small noise steps:
          \[
            ...
            \qquad \varepsilon_k\overset{\mathrm{iid}}{\sim}\Normal(0,1).
          \]
  ...
  \begin{itemize}[<+->]
    ...
    \item \textbf{Generate from the Gaussian prior:}
      \[
        ...
        \xrightarrow{q_\theta}\;X^{K-1}
        \xrightarrow{q_\theta}\;\cdots
        \xrightarrow{q_\theta}\;
...
\begin{frame}{From diffusion models to diffusion samplers}
  ...
  \begin{itemize}[<+->]
    ...
    \item \textbf{Our diffusion sampler:} only the cost $C(x)$ is available.
      \[
        p_{\Tau}(x)=\frac{e^{-C(x)/\Tau}}{Z_{\Tau}},
        \qquad \min_\theta\KL\!\left(q_\theta(x^0)\|p_{\Tau}(x^0)\right).
      \]
    ...
    \item \textbf{The obstacle:} the marginal distribution
      $q_\theta(x^0)$ is intractable to evaluate.
      \[
        \underbrace{q_\theta(x^0)}_{\text{marginal}}
        =\int\underbrace{q_\theta(x^0,z)}_{\text{joint}}\,dz.
      \]
      ...
      \[
        \begin{aligned}
          ...
          &\quad+\underbrace{\E_{X\sim q}\!\left[
            \KL(q(z\mid X)\|p(z\mid X))\right]}_{\geq 0}.
        \end{aligned}
      \]
    \item \textbf{DPI: discarding $Z$ cannot increase KL.}
      \[
        \boxed{\KL(q(x)\|p(x))\leq\KL(q(x,z)\|p(x,z)).}
      \]
  \end{itemize}
\end{frame}
...
\begin{frame}{Apply the DPI to a diffusion path}
  ...
  \begin{itemize}[<+->]
    \setlength{\itemsep}{0.20cm}
    \item \textbf{Substitute $X=X^0$ and $Z=X^{1:K}$:}
      \[
    ...
    \item \textbf{The joint distributions are the full paths:}
      \[
        \begin{aligned}
          q_\theta(x^{0:K})
            &=q_K(x^K)\prod_{k=1}^{K}q_\theta(x^{k-1}\mid x^k),\\
          p_{\Tau}(x^{0:K})
            &=p_{\Tau}(x^0)\prod_{k=1}^{K}p(x^k\mid x^{k-1}).
        \end{aligned}
      \]
...
\begin{frame}[fragile]{Lesson 2 exercise: starter code}
  ...
  \uncover<1->{%
    \href{run:../L2-DiffusionSamplers/diffusion_sampler_student.ipynb}
      {\textcolor{BrandBlue}{\texttt{L2-DiffusionSamplers/diffusion\_sampler\_student.ipynb}}}
  }
  \vspace{0.10cm}
...
\begin{frame}{Reward defines a target over trajectories}
  ...
  \begin{block}<1->{Unnormalized joint target}
    For states $s_t$, actions $a_t$, horizon $T$, and temperature $\Tau>0$,
    \[
    ...
    \]
    Here $p(s_0)$ is the initial-state density and
    $p(s_{t+1}\mid s_t,a_t)$ describes the environment dynamics.
  \end{block}
  \uncover<2->{%
...
\begin{frame}{A policy induces a distribution over trajectories}
  ...
  \begin{columns}[T,onlytextwidth]
    \column{0.50\textwidth}
      ...
      \end{block}
      \uncover<2->{%
        \begin{block}{Fit the reward-defined target}
          \[
            \min_\theta\;\KL\!\left(
              q_\theta(a_{0:T})\|\pi(a_{0:T})\right).
          \]
        \end{block}
      }
...
\begin{frame}{DPI: actions to state--action trajectories}
  \only<1->{%
    \begin{tikzpicture}[remember picture,overlay]
      \node[anchor=north east,fill=white,rounded corners=1.2pt,
            inner xsep=2.8pt,inner ysep=1.8pt]
...
\begin{frame}{Infinite horizon: now let $T\to\infty$}
  \small
  \begin{itemize}[<+->]
    \item \textbf{We recover the standard maximum-entropy RL objective:}
      \[
        \boxed{
        ...
        \right].}
      \]
    \item \textbf{The familiar MaxEnt framework used by Soft Actor-Critic.}\\
...
\begin{frame}{Apply the maximum-entropy policy-gradient theorem}
  \small
  \setlength{\abovedisplayskip}{4pt}
  \setlength{\belowdisplayskip}{4pt}
  \begin{itemize}[<+->]
    \item \textbf{Discounted visitation under the current policy:}
      \[
        d^{q_\theta}(s)
        :=(1-\gamma)\sum_{t=0}^{\infty}\gamma^t
        \Pr_{q_\theta}(s_t=s).
      \]
    \item \textbf{The policy-gradient theorem applies:}
      \[
        \boxed{\nabla_\theta\mathcal J(\theta)
        =\frac{1}{1-\gamma}
        \E_{\substack{s\sim d^{q_\theta}\\a\sim q_\theta(\cdot\mid s)}}\!\left[
        \nabla_\theta\log q_\theta(a\mid s)
        \big(Q^{q_\theta}(s,a)-\Tau\log q_\theta(a\mid s)\big)
        \right].}
      \]
      $Q^{q_\theta}$ includes future rewards \emph{and} future entropy.
    \item \textbf{No differentiation through visitation or the critic:}
      the theorem accounts for their effect on the return.
  \end{itemize}
\end{frame}

% Local gradient identity: ICLR2027 draft, Sec. 2.1, Eqs. (2)--(3).
\begin{frame}{From policy gradients to a local VI surrogate}
  \footnotesize
  \setlength{\abovedisplayskip}{4pt}
  \setlength{\belowdisplayskip}{4pt}
  \begin{itemize}[<+->]
    \setlength{\itemsep}{0.20cm}
    \item \textbf{Freeze the rollout policy $q_{\thetar}$, its critic and visitation:}
      \[
        Q_r:=Q^{q_{\thetar}},\qquad d_r:=d^{q_{\thetar}},\qquad
        \pi_Q^r(a\mid s)\propto\exp\!\left(Q_r(s,a)/\Tau\right).
      \]
    \item \textbf{Reverse the log-derivative trick to obtain the actor loss:}
      \[
        \begin{aligned}
          \mathcal L_{\mathrm{ME}}(\theta)
          &=\frac{1}{1-\gamma}
            \E_{\substack{s\sim d_r\\a\sim q_\theta(\cdot\mid s)}}\!\left[
            \Tau\log q_\theta(a\mid s)-Q_r(s,a)\right]\\
          &=\frac{\Tau}{1-\gamma}\,
            \E_{s\sim d_r}\!\left[
            \KL\!\left(q_\theta(\cdot\mid s)\|\pi_Q^r(\cdot\mid s)\right)
            \right]+\mathrm{const}.
        \end{aligned}
      \]
    \item \textbf{Exact at the first iterate on this rollout batch, $\theta^{(0)}=\thetar$:}
      \[
        \boxed{\left.\nabla_\theta\mathcal L_{\mathrm{ME}}(\theta)\right|_{\theta=\thetar}
        =-\left.\nabla_\theta\mathcal J(\theta)\right|_{\theta=\thetar}.}
      \]
      Exact values and expectations; later actor iterates generally use an approximate gradient.
  \end{itemize}
\end{frame}

\begin{frame}{The soft Bellman equations provide the critic}
  \small
  \begin{columns}[T,onlytextwidth]
    \column{0.58\textwidth}
      \begin{block}<1->{Evaluate the rollout policy: action value}
        \[
          Q_r(s,a)
          =R_{\mathrm{env}}(s,a)
      ...
      \begin{block}<2->{Soft state value}
        \[
          V_r(s)
          =\E_{a\sim q_{\thetar}(\cdot\mid s)}\!\left[
          Q_r(s,a)-\Tau\log q_{\thetar}(a\mid s)
          \right].
        \]
      \end{block}
      \begin{alertblock}<3->{Policy evaluation $\to$ actor updates}
        $V_r:=V^{q_{\thetar}}$. Estimate the soft critic under $q_{\thetar}$,
        then freeze it while optimizing the local actor loss.
      \end{alertblock}
      ...
      \begin{tikzpicture}[x=0.82cm,y=0.84cm,font=\scriptsize,
        ...
        \uncover<1->{%
          ...
          \draw[-{Stealth[length=7pt]},BrandBlue,line width=1.8pt]
            (s) -- node[above,text=Ink] {$a\sim q_{\thetar}(\cdot\mid s)$} (sp);
        ...
        \uncover<3->{%
          ...
          \node[text=TrailDark,font=\bfseries,align=center] at (2.18,0.65)
            {$-\Tau\log q_{\thetar}(a\mid s)$\\rewards entropy};
        }
      \end{tikzpicture}
  \end{columns}
\end{frame}

\begin{frame}{Multiple actor updates require a trust region}
  \footnotesize
  \begin{columns}[T,onlytextwidth]
    \column{0.62\textwidth}
      \begin{block}{Several updates on the same rollout batch}
        \begin{itemize}
          \item Start at $\theta^{(0)}=\thetar$; freeze $d_r$ and $Q_r$.
          \item Take several steps on $\mathcal L_{\mathrm{ME}}$.
          \item Keep every candidate close to the \textbf{original rollout policy}.
          \item Later gradients remain a local approximation.
        \end{itemize}
      \end{block}

      \begin{alertblock}{TRPO improvement condition}
        Accept a candidate when its surrogate gain exceeds the bound's
        distribution-shift penalty. A sufficiently small trust region controls
        that error; then refresh rollouts and the critic.
      \end{alertblock}
    ...
    \column{0.34\textwidth}
      ...
      \begin{tikzpicture}[x=0.65cm,y=0.70cm,font=\scriptsize,
        ...
        \node[text=BrandBlue,font=\bfseries,align=center]
          at (2.70,0.82) {surrogate remains\\reliable};
  ...
  \centering
  \(\boxed{%
    \max_s\KL\!\left(q_{\thetar}(\cdot\mid s)\|q_{\theta^{(j)}}(\cdot\mid s)\right)
  ...
  {\scriptsize
    ...
    Population guarantees use exact values and worst-state bounds;
    sampled KL constraints are a practical approximation.}
\end{frame}
